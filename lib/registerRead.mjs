...
export async function readRegister(file) {
  ...
  const entries = rows.slice(1).map((row) => ({
    ...
    ...entry,
    /* A row can legitimately reach the register with no contact details: a
       session queued offline before the details were captured still carries a
       real score, and dropping it would quietly disqualify that player. But an
       unreachable WINNER is a problem you want to find before you are holding a
       microphone, not after - so it is flagged everywhere the row is shown. */
    hasContact: Boolean(entry.phone && entry.bitsId)
  }));
